...
function ProductPicker({ products, onSelect, onClose }) {
  ...
  const filtered = products.filter(p => (!cat || p.category === cat) && ((p.name||"").includes(q) || String(p.code||"").includes(q))).sort((a, b) => String(a.code||"").localeCompare(String(b.code||""), "ja", {numeric:true}));
  return (
    <div style={s.modal} onClick={onClose}>
...
function DeliveryForm({ clients, products, deliveries, clientPrices, divisions, company, invoices, balances, onSave, onClose, editing }) {
  ...
  const filteredClients = clientSearch ? clients.filter(c => c.name?.includes(clientSearch) || c.code?.includes(clientSearch)) : clients;
    ...
    <div>
      ...
      {viewMode === "client" && (() => {
        ...
        <div style={{ ...s.card, overflowX: "auto" }} className="print-area">
          <style>{`@media print { @page { size: A4 landscape; margin: 8mm; } body * { visibility: hidden; } .print-area, .print-area * { visibility: visible; } .print-area { position: absolute; left: 0; top: 0; width: 100%; } .no-print { display: none !important; } table { font-size: 8px; width: 100%; } th, td { padding: 2px 3px !important; white-space: nowrap; } th:first-child, td:first-child { max-width: 120px; overflow: hidden; text-overflow: ellipsis; } }`}</style>
